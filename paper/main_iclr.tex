...
% PARITY TOGGLE (decided by the 10k convergence pair): true = rotation
% matches dense at 10k -> positive headline ("half-cost training");
% false = scoped to compute-limited regimes.
\newif\ifparity
\paritytrue
...
accuracy benefit at either 11M or 150M scale. Measuring the field
directly---the per-token gradient of the loss with respect to each token's
compute gate---explains why: the field carries transient structure early in
training and is flat at convergence, so there is nothing for any allocator,
learned or supervised, to exploit. \ifparity The constructive result:
training with \emph{token-level FFN dropout}---each token receives full
feed-forward compute only inside randomly rotating windows---matches dense
quality at \todoX{half} the FFN training FLOPs on 11M and 150M models and on
\todoX{1.4B} language-model fine-tuning, with dense deployment requiring no
architecture change. \else In the compute-limited regime the rotated recipe
recovers most but not all of dense quality; the parity claim is scoped
accordingly. \fi The contribution is a measurement methodology
(permutation-controlled ordering gaps, allocation headroom, the value-field
life cycle) and a free training recipe the adaptive-compute literature
skips: the baseline that beats every allocator is random.
\end{abstract}
...
token's FFN gate---and find it has a life cycle: structured transiently
early in training, consumed by it, and flat at convergence
(\S\ref{sec:measure}). A permutation-controlled oracle shows the converged
field has no realizable headroom, and a causal single-stage test shows that
\emph{chasing} the field online is worse than random allocation of the same
budget. What works is the degenerate allocator everyone skips: train with
\emph{token-level FFN dropout}---each token receives full FFN compute only
inside randomly rotating windows---and deploy dense. At matched tokens this
matches dense quality at half the FFN training FLOPs
\ifparity across 11M, 150M, and 1.4B language-model fine-tuning \else at
11M, with the 150M and 1.4B parity claims scoped to the compute-limited
regime \fi, requires no labels or routers, and composes with any attention
architecture because it never touches attention.
