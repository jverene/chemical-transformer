...
150M models, and within \NlRotGapPct\% on 1.4B language-model fine-tuning,
...
at 150M; $-7.8\%$ at 1.4B natural-language fine-tuning)
...
\subsection{Natural language: the recipe transfers at 1.4B}
\label{sec:nl}

Pythia-1.4b continue-trained on FineWeb-Edu (350M packed tokens, 3000
steps at sequence length 2048, seed 0; both arms share tokenizer, data, and
the accounting of \S\ref{sec:isoflops}). Dense reaches held-out loss
$\NlDenseLoss$ at $\NlDenseMFl$ billed MFLOPs/token. The rotation arm
trains at exactly \NlRotFlopsPct\% of the dense arm's billed training
FLOPs; deployed \emph{dense} (gates off, no architecture change) it
evaluates at $\NlRotLoss$: a ${+}\NlRotGapPct\%$ gap, inside the same
pre-registered $2$--$10\%$ band as the 150M testbed pair ($+5.3\%$).
Evaluated \emph{with the random ablation live} the rotation model is much
worse ($3.378$)---which is the point of the deployment claim: the trained
body wants full compute at inference, and gets it. Negative-space honesty:
the NL track here carries the recipe pair only; the oracle-measurement
machinery and the endogenous-signal taxonomy are testbed-scale results
