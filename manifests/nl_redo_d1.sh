...
if [ ! -f data/nl/webtext/meta.json ]; then
  $PY -m ct.nl.data --domain webtext --tokenizer EleutherAI/pythia-1.4b \
    --out-root data/nl --max-tokens 350000000 2>&1 | tee /workspace/phase_d1_1.log | tail -2
fi
test -f data/nl/webtext/train_tokens.npy || { echo "D1-1 FAILED"; exit 1; }

# D1-2: dense seed 1 (fresh body; ckpt feeds every fork below)
...
# D1-7: full-unfreeze ablation
$PY -m ct.nl.train_lm --domain webtext --model EleutherAI/pythia-1.4b \
  --method ours-gategrad --stage 2 --seed 1 --steps 2000 --batch-seqs 4 --grad-accum 2 \
  --full-unfreeze --suffix=_fullunf \
  --eval-every 250 --device cuda --data-root data/nl --out-root results-nl \
  2>&1 | tee /workspace/phase_d1_7.log | tail -3
test -f results-nl/webtext/ours-gategrad_fullunf_seed1.json || { echo "D1-7 FAILED"; exit 1; }

# D1-8: per-layer oracle (same body, per-(token,layer) scores)
$PY -m ct.nl.oracle --domain webtext --model EleutherAI/pythia-1.4b \
  --ckpt results-nl/webtext/baseline_seed1.pt --data-root data/nl \
  --out-root results-nl --pool-seqs 20000 --batch-seqs 4 --device cuda --save-layers \
  2>&1 | tee /workspace/phase_d1_8.log | tail -4
test -f data/nl/webtext/oracle.npz || { echo "D1-8 FAILED"; exit 1; }

# D1-9: per-layer-target stage 2
$PY -m ct.nl.train_lm --domain webtext --model EleutherAI/pythia-1.4b \
  --method ours-gategrad --stage 2 --seed 1 --steps 2000 --batch-seqs 4 --grad-accum 2 \
  --per-layer-targets --suffix=_perlay \
  --eval-every 250 --device cuda --data-root data/nl --out-root results-nl \
  2>&1 | tee /workspace/phase_d1_9.log | tail -3
test -f results-nl/webtext/ours-gategrad_perlay_seed1.json || { echo "D1-9 FAILED"; exit 1; }
echo "MANIFEST D1 COMPLETE"
